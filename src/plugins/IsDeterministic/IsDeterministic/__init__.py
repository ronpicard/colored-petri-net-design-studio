...
class IsDeterministic(PluginBase):
    def main(self):
        ...
        def per_thing(node):
            ...
            try:
                ...
                if core.get_attribute(meta_tr, 'name')=='PlaceToTransition':
                    ...
                    try:
                        if src_path not in cur_state:
                            cur_state[src_path]=string_to_dict(tokens)
        ...
        for count in range(max_iter):
            ...
            #logger.info('Current->{0}END'.format(cur_state))
            for state in seen_states:
                #logger.info('Previous->{0}END'.format(state))
                if cur_state==state:
                    logger.info('reached repeat state. is deterministic')      
                    commit_info = self.util.save(root_node, self.commit_hash, 'master', 'Python plugin updated the model')
                    logger.info('wrote change')
                    return
            seen_states.append(cur_state)
            enabled=[]
        ...
        logger.info('hit max iterations. so far deterministic')
        commit_info = self.util.save(root_node, self.commit_hash, 'master', 'Python plugin updated the model')
        logger.info('wrote change')
